/**
 * Cut Fruit Page JavaScript (Updated)
...
async function initCutFruitPage() {
  try {
    ...
    // BƯỚC 2: Lọc sản phẩm cắt sẵn (tag: cut-fruit)
...
function applyFilters() {
  ...
  // Filter by price logic for Cut Fruit
...
function createProductCard(product) {
  ...
  if (hasSale) {
    ...
    if (product.tags?.includes("cut-fruit"))
...
function addToCart(productId, productName) {
  ...
  if (typeof CartManager !== "undefined") {
    ...
    alert("Lỗi: Hệ thống chưa tải xong. Vui lòng thử lại!");
  }
}
